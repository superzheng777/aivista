...
class StyleResponseDto {
    ...
    @ApiProperty({
        description: '风格提示词',
        example: 'neon lights, high tech, low life...',
    })
    prompt: string

    @ApiProperty({
        description: '风格描述',
        example: '赛博朋克风格：霓虹灯、高科技...',
        required: false,
    })
    description?: string
...
class StatsResponseDto {
    ...
    @ApiProperty({
        description: '是否已初始化（数据库文件存在且表有数据）',
        example: true,
    })
    initialized: boolean
...
class AddStyleRequestDto {
    ...
    @ApiProperty({
        description: '风格提示词',
        example: 'impressionist painting, soft brushstrokes...',
    })
    @IsString()
    prompt: string
...
export class KnowledgeController {
    ...
    async addStyle(
        @Body() styleData: AddStyleRequestDto
    ...
    async deleteStyles(
        @Body() deleteDto: DeleteStylesDto
