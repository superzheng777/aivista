...
>
 * 这样可以避免未使用的服务要求 API key 导致启动失败: - 当 NestJS 启动时，它会实例化 providers 列表里的所有类。即使你只想用阿里云，NestJS 也会去 new OpenAIEmbedding()。如果你没配 OpenAI 的 Key，OpenAIEmbedding 的构造函数就会报错（抛出 Error）。结果：整个后端程序直接崩溃，无法启动，即便你根本不用那个服务。
 */
@Module({
