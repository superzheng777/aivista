...
export class EmbeddingProviderFactory {
    ...
    create(): IEmbeddingService {
        ...
        //（由于env没有配置MBEDDING_PROVIDER，因此EMBEDDING_PROVIDER使用的服务与LLM_PROVIDER保持一致）
        // 注意，不是模型相同，是使用同一个品牌的服务
        const embeddingProvider =
            this.configService.get<string>('EMBEDDING_PROVIDER')
        const llmProvider =
            this.configService.get<string>('LLM_PROVIDER') || 'aliyun'
        const provider = embeddingProvider || llmProvider

        switch (provider.toLowerCase()) {
            case 'aliyun':
                this.cachedService = new AliyunEmbeddingService(
                    this.configService
                )
                break
            case 'openai':
            case 'deepseek':
                ...
                this.cachedService = new OpenAIEmbeddingService(
                    this.configService
                )
                break
            default:
