...
class EnvironmentVariables {
    ...
    // 布尔型配置 - 统一使用 string 类型，避免依赖 enableImplicitConversion
    @IsString()
    @IsOptional()
    CRITIC_USE_LLM?: string
    ...
    @IsString()
    @IsOptional()
    FORCE_INIT_KNOWLEDGE_BASE?: string
...
export function validateEnvironment(config: Record<string, unknown>) {
    // 转换为 EnvironmentVariables 类实例，仅仅把 config 中的属性转换为 EnvironmentVariables 类的属性，不做@IsNotEmpty()等验证
    ...
    if (validatedConfig.LLM_PROVIDER === LlmProvider.ALIYUN) {
        if (!validatedConfig.DASHSCOPE_API_KEY) {
            throw new Error(
                'DASHSCOPE_API_KEY is required when LLM_PROVIDER=aliyun'
            )
        }
    ...
    const embeddingProvider =
        validatedConfig.EMBEDDING_PROVIDER || validatedConfig.LLM_PROVIDER
    if (embeddingProvider === 'aliyun') {
        if (!validatedConfig.DASHSCOPE_API_KEY) {
            throw new Error(
                'DASHSCOPE_API_KEY is required when EMBEDDING_PROVIDER=aliyun (or LLM_PROVIDER=aliyun)'
            )
        }
    } else if (
        ...
        if (
            ...
            !validatedConfig.DEEPSEEK_API_KEY
        ) {
            throw new Error(
                'OPENAI_API_KEY or DEEPSEEK_API_KEY is required when EMBEDDING_PROVIDER=openai/deepseek'
            )
        }
    }
