...
export class CriticNode {
    ...
    async execute(state: AgentState): Promise<Partial<AgentState>> {
        ...
        if (!state.generatedImageUrl || !state.intent) {
            this.logger.error(
                'Critic Node: generatedImageUrl and intent are required'
            )
            return {
                qualityCheck: {
        ...
        try {
            // 2. 获取配置参数
            const passThreshold =
                this.configService.get<number>('CRITIC_PASS_THRESHOLD') ?? 0.7
            const useLlm = parseBooleanEnv(
                this.configService.get<string>('CRITIC_USE_LLM')
            )
            const maxRetryCount =
                this.configService.get<number>('MAX_RETRY_COUNT') ?? 3
            const currentRetryCount = state.metadata?.retryCount || 0
            // this.logger.log(
            //     `Critic Node: useLlm: ${this.configService.get<string>('CRITIC_USE_LLM')}`
            // )
            this.logger.log(
            ...
            if (useLlm) {
                // 使用 LLM 进行真实审查（方案 A）
                qualityCheck = await this.performLlmQualityCheck(
                    state,
                    passThreshold
                )
            } else {
                // 使用简化逻辑（方案 B，MVP 版本）
                qualityCheck = this.performSimpleQualityCheck(
                    state,
                    passThreshold
                )
            }
            ...
            // 确保score是有效数字
            if (
                typeof qualityCheck.score !== 'number' ||
                isNaN(qualityCheck.score)
            ) {
                this.logger.warn(
            ...
            const shouldRetry =
                !qualityCheck.passed && currentRetryCount < maxRetryCount
            const newRetryCount = shouldRetry
                ? currentRetryCount + 1
                : currentRetryCount

            // 5. 推送思考日志
            const passedText = qualityCheck.passed ? '通过' : '未通过'
            const retryText = shouldRetry
                ? `，将进行第 ${newRetryCount} 次重试`
                : ''
            const thoughtLog = {
                node: 'critic',
            ...
            this.logger.error(
                `Critic Node error: ${error.message}`,
                error.stack
            )
...
请评估这张图片是否满足用户需求。`
        ...
        try {
            const result =
                await this.llmService.chatWithJson<QualityCheckResult>(
                    messages,
                    {
                        temperature: 0.3,
                        jsonMode: true,
