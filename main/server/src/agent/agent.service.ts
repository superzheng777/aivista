        ...
        try {
            ...
            for await (const chunk of stream) {
                ...
                for (const [nodeName, stateUpdate] of Object.entries(chunk)) {
                    this.logger.log(
                        `Node ${nodeName} executed, updating state...`
                    )
                    ...
                    if (update.thoughtLogs && update.thoughtLogs.length > 0) {
                        for (const log of update.thoughtLogs) {
                            console.log(
                                `[AgentService] 推送思考日志: ${log.node} - ${log.message}`
                            )
                    ...
                    // 多轮续写时跳过：实际使用 Executor 的合并 prompt，由 Executor 推送 EnhancedPromptView
                    if (
                        nodeName === 'rag' &&
                    ...
                    if (nodeName === 'critic' && update.qualityCheck) {
                        ...
                        const retryCount =
                            update.metadata?.retryCount ||
                            currentState.metadata?.retryCount ||
                            0
