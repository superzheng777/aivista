...
import {
    ActionPanelProps as BaseActionPanelProps,
    ActionItem,
        ...
        if (action.id === 'regenerate_btn') {
            window.dispatchEvent(
                new CustomEvent('genui-action', {
                    detail: { actionId: 'regenerate_btn' },
                })
            )
            return
        }
        //触发蒙版编辑模式，传递当前图片URL
        if (action.id === 'edit_mask_btn' && latestImageUrl) {
            window.dispatchEvent(
                new CustomEvent('genui-action', {
                    detail: {
                        actionId: 'edit_mask_btn',
    ...
    return (
        <div className="flex items-center gap-2 pt-4 pb-2">
            {actions.map((action) => {
                ...
                const isDisabled =
                    (action.id === 'download_btn' ||
                        action.id === 'preview_btn' ||
                        action.id === 'edit_mask_btn') &&
                    !latestImageUrl
