...
export class MessageService {
    ...
    static async saveUserMessage(
        ...
        try {
            ...
            console.error(
                '[MessageService] Failed to save user message:',
                error
            )
            throw error
        }
    ...
    static async loadSessionMessages(sessionId: string): Promise<Message[]> {
        try {
            ...
            console.error(
                '[MessageService] Failed to load session messages:',
                error
            )
            return []
        }
    ...
    static async deleteSessionMessages(sessionId: string): Promise<void> {
        try {
            ...
            console.error(
                '[MessageService] Failed to delete session messages:',
                error
            )
            throw error
        }
    ...
    private static async updateSessionInfo(sessionId: string): Promise<void> {
        try {
            ...
            await db.sessions.update(sessionId, {
                ...
                lastMessage:
                    lastMessagePreview +
                    (lastUserMessage && lastUserMessage.content.length > 50
                        ? '...'
                        : ''),
            })
        } catch (error) {
            console.error(
                '[MessageService] Failed to update session info:',
                error
            )
            // 不抛出错误，避免影响消息保存
        }
    ...
    private static extractImageUrl(
        ...
        if (imageView && imageView.props && 'imageUrl' in imageView.props) {
            return imageView.props.imageUrl as string // as string 断言 确保 imageUrl 是字符串类型
        }

        // 查找 SmartCanvas 组件
        const smartCanvas = components.find(
            (c) => c.widgetType === 'SmartCanvas'
        )
        if (
            smartCanvas &&
            smartCanvas.props &&
            'imageUrl' in smartCanvas.props
        ) {
    ...
    static async saveChatRequest(
        ...
        try {
            ...
            console.error(
                '[MessageService] Failed to save chat request:',
                error
            )
            throw error
        }
