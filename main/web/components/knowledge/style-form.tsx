...
export function StyleForm({
    ...
    const handleRemoveMetaEntry = (index: number) => {
        ...
        setMetaEntries(
            newEntries.length > 0 ? newEntries : [{ key: '', value: '' }]
        )
    }

    const handleUpdateMetaEntry = (
        index: number,
        field: keyof MetaEntry,
        value: string
    ) => {
        const newEntries = [...metaEntries]
    ...
    // 提交修改的tags和metadata（style和prompt必填，description选填，格式简单，直接绑定到onclick）
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        let finalMetadata = {}
        // JSON mode: Parse raw JSON string
            ...
            <div className="grid grid-cols-1 gap-5">
                ...
                <div className="space-y-2">
                    ...
                    <Input
                        ...
                        className={cn(
                            isSystem && 'bg-muted cursor-not-allowed'
                        )}
                    />
                    {isSystem && (
                    ...
                    <div className="flex flex-wrap gap-2 min-h-8 p-2 rounded-md border border-input bg-background focus-within:ring-1 focus-within:ring-ring transition-all">
                        ...
                        {formData.tags.map((tag) => (
                            <Badge
                                key={tag}
                                variant="secondary"
                                className="gap-1 px-2 py-0.5 h-6"
                            >
                                {tag}
                                <button
                                    type="button"
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Switch
                                ...
                                onCheckedChange={(checked) =>
                                    setMetaMode(checked ? 'advanced' : 'simple')
                                }
                                className="scale-75"
                            />
                        </div>
                    ...
                    {metaMode === 'simple' ? (
                        <div className="space-y-3 bg-muted/20 p-4 rounded-xl border border-dashed border-muted-foreground/20">
                            {metaEntries.map((entry, index) => (
                                <div
                                    key={index}
                                    className="flex gap-2 items-start animate-in fade-in slide-in-from-left-2 duration-200"
                                >
                                    <Input
                                        placeholder="键 (Key)"
                                        value={entry.key}
                                        onChange={(e) =>
                                            handleUpdateMetaEntry(
                                                index,
                                                'key',
                                                e.target.value
                                            )
