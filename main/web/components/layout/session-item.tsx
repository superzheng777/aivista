...
interface SessionItemProps {
    ...
    collapsed?: boolean
    /** 为 true 时编辑/删除按钮常驻显示（用于移动端 Sheet） */
...
export function SessionItem({
    ...
    // 编辑标题时的键盘事件处理（Enter 保存，Escape 取消）
    ...
    return (
        ...
        >
            ...
            {/* 标题和预览 */}
            <div className="flex-1 min-w-0 max-w-[130px]" title={displayTitle}>
                {isEditing ? (
                    <Input
                        value={editedTitle}
            ...
            {/* 操作按钮 - 悬停时显示（或 alwaysShowActions 时常驻），编辑时始终显示 */}
            <div
                className={cn(
                    'flex-shrink-0 items-center gap-1',
                    isEditing ? 'flex' : alwaysShowActions ? 'flex' : 'flex'
                )}
                onClick={(e) => e.stopPropagation()}
            >
                {isEditing ? (
