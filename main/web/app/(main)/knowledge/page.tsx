...
import { KnowledgeStats, StyleSearch, StyleList } from '@/components/knowledge'
...
export default function KnowledgePage() {
    ...
    const [showForm, setShowForm] = useState(false) // 是否显示创建表单弹窗
    const [showEditDialog, setShowEditDialog] = useState(false) // 是否显示编辑表单弹窗
    const [selectedStyle, setSelectedStyle] = useState<StyleData | null>(null) // 当前选中的样式
    const [dialogMode, setDialogMode] = useState<'view' | 'edit'>('view') // 当前弹窗模式（查看或编辑）
    const [styles, setStyles] = useState<StyleData[]>([]) // 所有样式列表
    const [loading, setLoading] = useState(false) // 是否加载中
    const [selectedIds, setSelectedIds] = useState<string[]>([]) // 选中的样式ID列表
    const [isSelectMode, setIsSelectMode] = useState(false) // 是否选择模式
    const [showDeleteConfirm, setShowDeleteConfirm] = useState(false) // 是否显示批量删除确认弹窗
    const [showSingleDeleteConfirm, setShowSingleDeleteConfirm] =
        useState(false) // 是否显示单个删除确认弹窗
    const [styleToDelete, setStyleToDelete] = useState<StyleData | null>(null) // 待删除的样式
    ...
    const handleBatchDeleteConfirm = async () => {
        //=====================！！！这里的操作好像是多余的===================
        // const selectedStyles = filteredStyles.filter((style) =>
        //     selectedIds.includes(style.id)
        // )
        // const result = await deleteStyles(
        //     selectedStyles.map((style) => style.id)
        // )

        const result = await deleteStyles(selectedIds)
    ...
    return (
        <div className="container mx-auto space-y-6">
            ...
            <BatchDeleteConfirm
                ...
                selectedStyles={filteredStyles.filter((style) =>
                    selectedIds.includes(style.id)
                )}
