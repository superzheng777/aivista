        ...
        (set, get) => ({
            ...
            selectSession: async (sessionId: string) => {
                try {
                    ...
                    console.error(
                        '[SessionStore] Failed to select session:',
                        error
                    )
                    throw error
                }
            ...
            deleteSession: async (sessionId: string) => {
                try {
                    ...
                    set((state) => {
                        ...
                        //?.id 可选链操作符，当 newSessions[0] 存在时返回其 id，否则返回 null(undefined||null = null)
                        const newCurrentId =
            ...
            deleteMultipleSessions: async (sessionIds: string[]) => {
                try {
                    ...
                    set((state) => {
                        ...
                        const newCurrentId = sessionIds.includes(
                            state.currentSessionId || ''
                        )
                            ? newSessions[0]?.id || null
                            : state.currentSessionId
                    ...
                    console.error(
                        '[SessionStore] Failed to delete sessions:',
                        error
                    )
                    throw error
                }
            ...
            updateSessionTitle: async (sessionId: string, title: string) => {
                try {
                    ...
                    console.log(
                        '[SessionStore] Updated session title:',
                        sessionId,
                        title
                    )
                } catch (error) {
                    console.error(
                        '[SessionStore] Failed to update session title:',
                        error
                    )
                    throw error
                }
            ...
            loadSessions: async () => {
                ...
                try {
                    ...
                    console.log(
                        '[SessionStore] Loaded sessions:',
                        sessions.length
                    )
                } catch (error) {
                    console.error(
                        '[SessionStore] Failed to load sessions:',
                        error
                    )
                    set({ isLoading: false })
                    throw error
            ...
            refreshCurrentSession: async () => {
                ...
                try {
                    const sessionRecord = await db.sessions.get(
                        currentSessionId
                    )
                    ...
                    if (sessionRecord) {
                        ...
                        console.log(
                            '[SessionStore] Refreshed current session:',
                            currentSessionId
                        )
                    }
                } catch (error) {
                    console.error(
                        '[SessionStore] Failed to refresh current session:',
                        error
                    )
                    throw error
                }
            ...
            clearAllSessions: async () => {
                try {
                    ...
                    console.error(
                        '[SessionStore] Failed to clear all sessions:',
                        error
                    )
                    throw error
                }
