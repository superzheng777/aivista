...
describe("generation task consumer adapter", () => {
  ...
  it("creates isolated prefetch-one channels for both Java-equivalent consumer pools", async () => {
    const channels = [channel(), channel(), channel()]; let index = 0;
    ...
    const consumer = new GenerationTaskConsumerService({ get: (key: string) => config[key] } as never,
      { consume: vi.fn() } as never, { consume: vi.fn() } as never);
    ...
    expect(channels[2]!.consume).toHaveBeenCalledWith("generation.image.transfer", expect.any(Function), { noAck: false });
...
const config: Record<string, unknown> = { AIVISTA_GENERATION_QUEUE_ENABLED: true,
  ...
  AIVISTA_TRANSFER_CONSUMER_CONCURRENCY: 1,
  AIVISTA_GENERATION_EXCHANGE: "aivista.generation.commands", AIVISTA_GENERATION_QUEUE_NAME: "generation.task.execute",
  AIVISTA_GENERATION_ROUTING_KEY: "generation.task.execute", AIVISTA_TRANSFER_QUEUE_NAME: "generation.image.transfer",
  AIVISTA_TRANSFER_ROUTING_KEY: "generation.image.transfer" };
